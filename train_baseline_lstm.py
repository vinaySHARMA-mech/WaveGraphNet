...
Usage:
  python train_baseline_lstm.py --split A --seed 42 --feature_type raw
  python train_baseline_lstm.py --split A --seed 42 --feature_type delta_e
"""
import os
...
from data.datasets import LstmDataset, LstmDeltaEDataset
from data.precompute import build_all_stats, LOOKBACK_POINTS, N_ATTENTION_FREQS, FIXED_FFT_BIN_INDICES
from models.baselines.lstm import LSTM_baseline, LSTM_baseline_deltae
...
def main():
    ...
    p.add_argument("--feature_type", default="raw", choices=["raw", "delta_e"],
                   help="'raw': per-pair 256-length frequency sequence. 'delta_e': the "
                        "66 delta_E scalars as one 66-timestep sequence (fairness ablation).")
    args = p.parse_args()
    ...
    label = "LSTM" if args.feature_type == "raw" else "LSTM (delta_e)"
    ...
    if args.feature_type == "raw":
        ...
        tr = DataLoader(LstmDataset(norm_data, train_ids, **ds_kw), batch_size=args.batch_size, shuffle=True)
        vl = DataLoader(LstmDataset(norm_data, val_ids, **ds_kw), batch_size=args.batch_size, shuffle=False)
        te = DataLoader(LstmDataset(norm_data, test_ids, **ds_kw), batch_size=args.batch_size, shuffle=False)
        model = LSTM_baseline(num_freqs=N_ATTENTION_FREQS, feature_dim_per_freq=2, num_sensor_pairs=num_pairs,
                              lstm_hidden_dim=args.lstm_hidden_dim, num_lstm_layers=args.num_lstm_layers,
                              decoder_hidden_dim=args.lstm_hidden_dim, output_dim=2,
        ...
        tr = DataLoader(LstmDeltaEDataset(norm_data, train_ids, **ds_kw), batch_size=args.batch_size, shuffle=True)
        vl = DataLoader(LstmDeltaEDataset(norm_data, val_ids, **ds_kw), batch_size=args.batch_size, shuffle=False)
        te = DataLoader(LstmDeltaEDataset(norm_data, test_ids, **ds_kw), batch_size=args.batch_size, shuffle=False)
        model = LSTM_baseline_deltae(num_paths=num_pairs, lstm_hidden_dim=args.lstm_hidden_dim,
                                     num_lstm_layers=args.num_lstm_layers, decoder_hidden_dim=args.lstm_hidden_dim,
